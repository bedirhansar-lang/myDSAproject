...
# Load the master table and sort by hotel/date for time-based features.
...
# Calendar features help the model learn seasonality.
...
# Past occupancy is used as an autoregressive signal.
for lag in [7, 14, 28]:
    df[f'occupancy_lag_{lag}'] = df.groupby('hotel_name')['occupancy_rate'].shift(lag)

# Bring in the strongest lagged Google Trends features from the EDA step.
...
# Baseline = seasonality + hotel identity + past occupancy.
baseline_features = [
    'hotel_name', 'month', 'day_of_week', 'week_of_year', 'doy_sin', 'doy_cos',
    'occupancy_lag_7', 'occupancy_lag_14', 'occupancy_lag_28'
]

# Trends model adds selected lagged search signals on top of the baseline.
...
for dataset_name, train_df, test_df, features in [
    ...
    for model_name, model in models:
        ...
        # Save feature importance for the random forest version.
...
# Evaluate results separately for each hotel.
by_hotel_rows = []
...
# Save the modeling-ready dataset and result tables.
...
# Find the best trends-augmented model to plot prediction examples.
...
# Write a compact markdown summary of the baseline modeling step.
summary_lines = []
