...
# Load the merged dataset and order it by hotel/date.
...
# Encode hotel name and create simple seasonal features.
...
# Add past occupancy values as autoregressive features.
for lag in [7, 14, 28]:
    df[f'occupancy_lag_{lag}'] = df.groupby('hotel_name')['occupancy_rate'].shift(lag)

# Rebuild the best lagged Trends features selected from earlier EDA work.
...
for name, feats in feature_sets.items():
    ...
    # Walk forward through time using two evaluation folds.
    ...
    for fold_name, train_dates, test_dates in folds:
        ...
        # Use the same random forest structure across all feature sets.
...
# Summarize average performance across the walk-forward folds.
summary_df = (
